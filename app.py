...
def main():
                ...
                for cluster_id, group in grouped:
                    ...
                    for i in range(len(clean_df)):
                        if i not in visited:
                            ...
                            # Validate the found group
                            if len(group_indices) > 1:
                                sub_df = clean_df.iloc[group_indices]
                                num_accounts = sub_df['account_id'].nunique()
    ...
    with tabs[3]:
        ...
        # Use the results from the Coordination Analysis (Tab 2)
        if not df_clustered_original.empty and 'coordination_groups_final' in locals():
            all_coord_hits = []
            for i, group in enumerate(coordination_groups_final):
                for post in group['posts']:
                    all_coord_hits.append({'account_id': post['account_id'], 'group_id': i})
            
            if all_coord_hits:
                df_coord_hits = pd.DataFrame(all_coord_hits)
                
                # Aggregate counts from the flagged coordination clusters
                account_stats = df_coord_hits.groupby('account_id').agg(
                    Coordinated_Posts=('group_id', 'count'),
                    Narratives=('group_id', 'nunique')
                ).reset_index()

                # MERGE WITH BASELINE: This pulls in the 'Actual_Total_Posts'
                risk_df = account_stats.merge(baseline_activity, on='account_id', how='left')
                
                # Calculate Ratio: Flagged / Total History
                risk_df['Inauthenticity Ratio'] = (risk_df['Coordinated_Posts'] / risk_df['Actual_Total_Posts']).fillna(0)
                
                def final_assessment(row):
                    acc_id_lower = str(row['account_id']).lower()
                    # 1. Check if it's a known news outlet
                    if any(key in acc_id_lower for key in LEGIT_MAP.keys()):
                        return "Verified Media (Neutral)"
                    # 2. Assign risk tiers to others
                    if row['Inauthenticity Ratio'] > 0.7 and row['Narratives'] >= 2:
                        return "High Risk (Bot Pattern)"
                    elif row['Inauthenticity Ratio'] > 0.4:
                        return "Medium Risk"
                    return "Low Risk"

                risk_df['Assessment'] = risk_df.apply(final_assessment, axis=1)
                
                # Sort to show most suspicious/active accounts first
                risk_df = risk_df.sort_values(by=['Narratives', 'Inauthenticity Ratio'], ascending=False)
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "Coordinated_Posts": "Flagged Hits",
                        "Actual_Total_Posts": "Total History",
                        "Inauthenticity Ratio": st.column_config.ProgressColumn(
                            "Inauthenticity %", help="Ratio of copy-paste posts vs total activity",
    ...
    with tabs[4]:
        ...
        st.markdown("Narratives grouped by content similarity. High-reach clusters are shown first.")
        
        if not all_summaries:
            st.info("No narrative clusters found.")
        else:
            # Sort summaries by reach as before
            for summary in sorted(all_summaries, key=lambda x: x['Total_Reach'], reverse=True):
                
                # --- CHANGE 1: REMOVE LIMITED CLUSTERS ---
                if "Limited" in summary.get('Emerging Virality', ''):
                    continue  # This skips the rest of the code for this cluster
                
                # --- CHANGE 2: CONSISTENT FONT SIZE (Using H3) ---
                st.markdown(f"### Cluster #{summary['cluster_id']} - {summary['Emerging Virality']}")
                
                col_reach, col_platform = st.columns([4,5])
                with col_reach:
                ...
                # Keep your original HTML/Table display for the cluster details
                with st.expander(f"View {len(summary['Posts_Data'])} Posts in Cluster"):
                    ...
                    posts_to_show['URL_Link'] = posts_to_show['URL'].apply(
                        lambda x: f'<a href="{x}" target="_blank">🔗 Link</a>' if pd.notna(x) and str(x).lower() != 'nan' else "No URL"
                    )
        ...
        # ---  TIKTOK & TELEGRAM NARRATIVES ---
        st.write("##") # Space
        st.divider()
        st.subheader("📱 TikTok & Telegram Narratives")
        st.markdown("Detailed tracking of narratives specifically appearing on TikTok and Telegram.")

        if all_summaries:
            # Combine all posts from all clusters to scan for platforms
            all_cluster_posts = pd.concat([s['Posts_Data'] for s in all_summaries])
            
            # Use your new inference function to ensure we catch the right labels
            all_cluster_posts['Inferred'] = all_cluster_posts['URL'].apply(infer_platform_from_url)
            
            # Filter specifically for your requested platforms
            monitor_df = all_cluster_posts[all_cluster_posts['Inferred'].isin(['TikTok', 'Telegram'])].copy()

            if not monitor_df.empty:
                # Add a column for narrative ID so you know which summary it belongs to
                # We do this by mapping back if possible, or showing the raw table
                st.dataframe(
                    monitor_df[['timestamp_share', 'Inferred', 'account_id', 'object_id', 'URL']],
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        "URL": st.column_config.LinkColumn("View Post", display_text="🔗 Open"),
                ...
                st.info("No TikTok or Telegram posts were found within the clustered narratives.")
    st.sidebar.markdown("---")
